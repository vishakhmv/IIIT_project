...
The testing pipeline uses the same `TESSSpeechDataset` preprocessing pipeline used during training to ensure consistent feature generation and inference behavior. :contentReference[oaicite:0]{index=0}
...
to preserve stable evaluation consistency. :contentReference[oaicite:1]{index=1}
...
The checkpoint stores the learned parameters obtained during validation-based training. :contentReference[oaicite:2]{index=2}
...
The predicted emotion corresponds to the class with the highest probability score. :contentReference[oaicite:3]{index=3}
